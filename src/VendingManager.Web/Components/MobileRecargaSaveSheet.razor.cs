...
public partial class MobileRecargaSaveSheet : ComponentBase, IDisposable, IAsyncDisposable
{
    ...
    /// <summary>
    /// Called with the captured photo when the user taps "Guardar carga".
    /// Parent should call slot-batch then foto-guia, then navigate to Resumen.
    /// </summary>
    [Parameter, EditorRequired] public EventCallback<CapturedPhoto> OnSaveAndOverview { get; set; }

    /// <summary>
    /// Called with the captured photo when the user taps "Guardar y cargar otra máquina".
    /// Parent should call slot-batch then foto-guia, then navigate to PickMachine.
    /// </summary>
    [Parameter, EditorRequired] public EventCallback<CapturedPhoto> OnSaveAndPickAnother { get; set; }
    ...
    private async Task HandleSaveAndOverview()
    {
        ...
        try
        {
            ...
            await OnSaveAndOverview.InvokeAsync(photo);
        }
        finally
        {
            _uploading = false;
    ...
    private async Task HandleSaveAndPickAnother()
    {
        ...
        {
            var photo = new CapturedPhoto(_capturedBytes, _capturedContentType!, _capturedFileName!);
            await OnSaveAndPickAnother.InvokeAsync(photo);
        }
        finally
        {
            _uploading = false;
