...
public class AdminCredentialGuardHostedService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "SECURITY: could not evaluate the '{Username}' user's password state because " +
                    "its stored hash is null or empty. Skipping the default-credential check.",
            ...
            try
            {
                isDefaultPassword = BCrypt.Net.BCrypt.Verify(DefaultAdminPassword, admin.PasswordHash);
            }
            catch (BCrypt.Net.SaltParseException)
            {
                _logger.LogWarning(
                    "SECURITY: could not evaluate the '{Username}' user's password state because " +
                    "its stored hash is not a valid BCrypt hash. Skipping the default-credential check.",
                    DefaultAdminUsername);
                return;
            }
        ...
        {
            // Best-effort guard: a transient DB failure at boot must never abort
            // application startup. Mirrors the catch+log behavior around
            // Database.Migrate() in Program.cs for the same class of failure.
            _logger.LogError(ex,
                "SECURITY: the admin credential guard failed to run and was skipped for this boot.");
        }
    }
